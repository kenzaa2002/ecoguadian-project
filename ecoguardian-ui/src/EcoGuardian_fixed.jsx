...
const APPLIANCES = [
  ...
  const [usageProfile, setUsageProfile] = useState("average");

  const selectedHouse = dashboards.find(d => d.id === form.dashboard_id) || null;
  const houseSizeLocked = Boolean(selectedHouse);

  const setField = (k, v) => setForm(p => ({ ...p, [k]: v }));

  const handleHouseChange = (value) => {
    const dashboardId = value ? Number(value) : null;
    const house = dashboards.find(d => d.id === dashboardId);

    setForm(p => ({
      ...p,
      dashboard_id: dashboardId,
      Household_Size: house ? house.household_size : p.Household_Size,
    }));
  };

  const applyUsageProfile = (profile) => {
    setUsageProfile(profile);

    // The backend model still expects DayOfWeek and Is_Weekend.
    // For a 3-month forecast, the UI uses a clearer usage profile and maps it
    // to representative model inputs.
    if (profile === "weekend") {
      setForm(p => ({ ...p, DayOfWeek: 6, Is_Weekend: 1 }));
    } else {
      setForm(p => ({ ...p, DayOfWeek: 1, Is_Weekend: 0 }));
    }
  };
  ...
  const usageProfiles = [
    { key: "average", label: "📊 Average", hint: "Best for 3-month forecast" },
    { key: "weekday", label: "🏢 Weekday", hint: "Normal working day" },
    { key: "weekend", label: "🏠 Weekend", hint: "More time at home" },
  ];
        ...
        <div className="card fade-up" style={{ padding: 24, borderColor: "rgba(34,211,238,.22)" }}>
          ...
          {dashboards.length > 0 && (
            <div className="input-group">
              ...
              <select className="input" value={form.dashboard_id || ""} onChange={e => handleHouseChange(e.target.value)}>
                <option value="">No specific house</option>
                {dashboards.map(d => <option key={d.id} value={d.id}>{d.name} — {d.household_size} people</option>)}
              </select>
              {selectedHouse && (
                <p style={{ color: G.text3, fontSize: 11, marginTop: 6 }}>
                  Using saved house data: household size = {selectedHouse.household_size}{selectedHouse.address ? `, address = ${selectedHouse.address}` : ""}.
                </p>
              )}
            </div>
          )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div className="input-group">
              <label className="label">Household size {houseSizeLocked ? "(from selected house)" : ""}</label>
              <input
                className="input"
                type="number"
                min={1}
                ...
                disabled={houseSizeLocked}
                onChange={e => setField("Household_Size", +e.target.value)}
                style={houseSizeLocked ? { opacity: .7, cursor: "not-allowed" } : undefined}
              />
              {houseSizeLocked && <p style={{ color: G.text3, fontSize: 11, marginTop: 6 }}>Automatically filled from the saved house.</p>}
            </div>
            <div className="input-group">
          ...
          <div className="input-group">
            ...
            <p style={{ color: G.text3, fontSize: 12, marginBottom: 10 }}>
              For a 3-month forecast, the app uses a general usage profile instead of asking for one specific day.
            </p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
              {usageProfiles.map(profile => (
                <button
                  key={profile.key}
                  className={`btn ${usageProfile === profile.key ? "btn-primary" : "btn-ghost"}`}
                  style={{ justifyContent: "center", fontSize: 12, flexDirection: "column", gap: 2, padding: "9px 6px" }}
                  onClick={() => applyUsageProfile(profile.key)}
                >
                  <span>{profile.label}</span>
                  <span style={{ fontSize: 10, opacity: .7 }}>{profile.hint}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="fade-in" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div className="card" style={{ padding: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                ...
                <span className="badge badge-green">{result.appliance_label === "Multi-appliance" ? `${result.appliance_breakdown?.length || 0} appliances` : result.appliance_label}</span>
              </div>
            ...
            {result.appliance_breakdown?.length > 0 && (
              <div className="card" style={{ padding: 20 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                  <h3 style={{ fontSize: 14, fontWeight: 600 }}>Forecast by appliance</h3>
                  <span className="badge badge-green">Total: {result.appliance_breakdown.length}</span>
                </div>

                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {result.appliance_breakdown.map((item, idx) => (
                    <div key={`${item.appliance_label}-${idx}`} style={{ background: G.bg2, borderRadius: 10, padding: 12, border: `1px solid ${G.border}` }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                        <div style={{ fontSize: 13, fontWeight: 700 }}>{item.appliance_label}</div>
                        <div className="mono" style={{ fontSize: 12, color: G.accent }}>× {item.quantity}</div>
                      </div>

                      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
                        {item.forecasts.map(f => (
                          <div key={f.month} style={{ background: G.card, borderRadius: 8, padding: 8, textAlign: "center" }}>
                            <div className="mono" style={{ fontSize: 10, color: G.text3 }}>{f.label}</div>
                            <div className="syne" style={{ fontSize: 15, fontWeight: 700, color: G.text }}>{f.kwh.toFixed(0)} kWh</div>
                            <div className="mono" style={{ fontSize: 10, color: G.warn }}>{f.cost_tnd.toFixed(2)} TND</div>
                          </div>
                        ))}
                      </div>

                      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 10, fontSize: 11, color: G.text3 }}>
                        <span>Avg: <b style={{ color: G.text }}>{item.avg_monthly_kwh.toFixed(1)} kWh</b></span>
                        <span>Peak: <b style={{ color: G.text }}>{item.peak_kwh.toFixed(1)} kWh</b></span>
                        <span>Cost: <b style={{ color: G.warn }}>{item.total_cost_tnd.toFixed(2)} TND</b></span>
                      </div>
                    </div>
                  ))}
                </div>
...
function WeeklyPage({ dashboards }) {
  ...
  const selectedWeeklyHouse = dashboards.find(d => d.id === form.dashboard_id) || null;
  const weeklyHouseSizeLocked = Boolean(selectedWeeklyHouse);

  const handleWeeklyHouseChange = (value) => {
    const dashboardId = value ? Number(value) : null;
    const house = dashboards.find(d => d.id === dashboardId);
        ...
        <div className="card fade-up" style={{ padding: 20 }}>
          ...
          {dashboards.length > 0 && (
            <div className="input-group">
              ...
              <select className="input" value={form.dashboard_id || ""} onChange={e => handleWeeklyHouseChange(e.target.value)}>
                <option value="">No specific house</option>
                {dashboards.map(d => <option key={d.id} value={d.id}>{d.name} — {d.household_size} people</option>)}
              </select>
              {selectedWeeklyHouse && (
                <p style={{ color: G.text3, fontSize: 11, marginTop: 6 }}>
                  Household size is filled from the selected house.
                </p>
              )}
            </div>
          )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div className="input-group">
              <label className="label">Household size {weeklyHouseSizeLocked ? "(from selected house)" : ""}</label>
              <input
                className="input"
                type="number"
                min={1}
                max={50}
                value={form.household_size}
                disabled={weeklyHouseSizeLocked}
                onChange={e => setForm(p => ({ ...p, household_size: +e.target.value }))}
                style={weeklyHouseSizeLocked ? { opacity: .7, cursor: "not-allowed" } : undefined}
              />
            </div>
            <div className="input-group">
